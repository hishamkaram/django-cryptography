...
class TestSigner(SimpleTestCase):
    def test_signature(self):
        ...
        for s in (
            ...
            self.assertEqual(
                signer.signature(s), signing.base64_hmac(signer.salt + 'signer', s, 'predictable-secret').decode()
            )
...
class TestBytesSigner(SimpleTestCase):
    def test_signature(self):
        ...
        for s in (
            ...
            self.assertEqual(
                signer.signature(s), signing.salted_hmac(signer.salt + 'signer', s, 'predictable-secret').finalize()
            )
            self.assertNotEqual(signer.signature(s), signer2.signature(s))
    ...
    def test_signature_with_salt(self):
        ...
        self.assertEqual(
            signer.signature('hello'),
            signing.salted_hmac('extra-salt' + 'signer', 'hello', 'predictable-secret').finalize(),
        )
        self.assertNotEqual(
    ...
    def test_works_with_non_ascii_keys(self):
        ...
        self.assertEqual(
            b'foo\xb5\x8a\xc47\x19\xaeN\xdcMT\x83{PAb\r' b'B\xf3\xd2i\xd1P\x94\xeb^\xc7(\xb4\xd3\x92' b'\xd3\xf4',
            s.sign('foo'),
        )
...
class TestFernetSigner(SimpleTestCase):
    def test_fernet_signer(self):
        ...
        with freeze_time(123456800 + signing._MAX_CLOCK_SKEW):
            ...
            # max_age parameter can also accept a datetime.timedelta object
            self.assertEqual(signer.unsign(ts, ttl=datetime.timedelta(seconds=11)), value)
            with self.assertRaises(signing.SignatureExpired):
